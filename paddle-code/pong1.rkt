...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname pong1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
;; Helper function to determine if ball reaches right edge
(define (ball-at-edge? x-pos)
  (>= (+ x-pos BALL-RADIUS) WIDTH))

;; Helper function to calculate new x position
(define (new-x-position current-x)
  (+ current-x BALL-SPEED))

;; Helper function to adjust x position if it exceeds boundary
(define (adjust-x-position x-pos)
  (if (ball-at-edge? x-pos)
      (- WIDTH BALL-RADIUS)
      x-pos))


;EXTRA CREDIT
;rewrite this function so that its easier to read. 

(define (move-ball state)
  (make-game
   (game-paddle-pos state)
   (make-pos 
    (adjust-x-position (new-x-position (pos-x (game-ball-pos state))))
    (pos-y (game-ball-pos state)))))
